...
describe('Friendship API Service', () => {
  const mockCharacter1: CharacterSheet = {
    ...
    characterData: {
      ...
      socialSkills: [],
      passiveTraits: [],
      weakness: { name: 'Laser Pointer', description: 'Cannot resist red dots' },
      timeModifiers: [],
    },
    petPhoto: null,
  };
  ...
  describe('validateFriendshipInput', () => {
    ...
    test('rejects missing first character', () => {
      const result = validateFriendshipInput(null as unknown as CharacterSheet, mockCharacter2);
      
      expect(result.isValid).toBe(false);
      expect(result.errorMessage).toBe('Both characters are required to generate a friendship report');
    });

    test('rejects missing second character', () => {
      const result = validateFriendshipInput(mockCharacter1, null as unknown as CharacterSheet);
    ...
    test('rejects identical character IDs', () => {
      ...
      expect(result.isValid).toBe(false);
      expect(result.errorMessage).toBe('Please select two different characters for the friendship report');
    });
  ...
  describe('getCharacterDisplayNames', () => {
    test('extracts names from character objects', () => {
      const [name1, name2] = getCharacterDisplayNames(mockCharacter1, mockCharacter2);
