...
export function EnvironmentalPhase({
  ...
  useEffect(() => {
    ...
    // Show first power modifiers after 1.5 seconds
    const modifier1Timer = setTimeout(() => {
      setShowModifiers(1);
    }, 1500);

    // Show second power after 3 seconds
    const power2Timer = setTimeout(() => {
      setCurrentPower(2);
      setShowModifiers(1); // Reset modifiers
    }, 3000);

    // Show second power modifiers after 4.5 seconds
  ...
  return (
    <div className={`environmental-section ${isActive ? 'animate-in' : 'visible'} mb-4`}>
