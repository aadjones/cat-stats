...
const PetPersonalityAnalyzer = () => {
  ...
  const questions = [
    {
      ...
      options: [
        { value: 'A', text: 'Immediately runs up with a toy or starts showing off', stats: { charisma: 25, boldness: 10 }},
        { value: 'B', text: 'Approaches slowly, sniffs cautiously, then decides if they\'re worthy', stats: { wisdom: 15, cunning: 10 }},
        { value: 'C', text: 'Watches from a safe distance until they prove themselves', stats: { stealth: 20, wisdom: 15 }},
        { value: 'D', text: 'Hides completely until the person leaves', stats: { charisma: -10, stealth: 25 }}
      ]
    },
    {
      id: 'problem_solving',
      ...
      options: [
        { value: 'A', text: 'Find the most acrobatic route - climb, jump, or parkour their way there', stats: { agility: 20, boldness: 15 }},
        { value: 'B', text: 'Study the situation carefully, then execute a precise plan', stats: { wisdom: 25, cunning: 10 }},
        { value: 'C', text: 'Use pure persistence - stare, whine, or paw until you help', stats: { charisma: 20, resolve: -10 }},
        { value: 'D', text: 'Find a sneaky workaround when no one\'s watching', stats: { cunning: 20, stealth: 15 }}
      ]
    },
    {
      id: 'stress_response',
      question: 'During stressful situations (loud noises, vet visits), your pet...',
      options: [
        { value: 'A', text: 'Gets agitated and protests - this is an outrage!', stats: { resolve: -15, boldness: 10 }},
        { value: 'B', text: 'Stays calm and handles it like a pro', stats: { resolve: 25, wisdom: 15 }},
        { value: 'C', text: 'Seeks comfort but remains relatively composed', stats: { resolve: 10, charisma: 10 }},
        { value: 'D', text: 'Completely shuts down or hides until it\'s over', stats: { stealth: 15, boldness: -10 }}
      ]
    },
    {
      id: 'favorite_spot',
      ...
      options: [
        { value: 'A', text: 'The highest perch - on top of furniture, appliances, etc.', stats: { agility: 15, territory: 'Vertical' }},
        { value: 'B', text: 'Hidden under blankets, beds, or cozy enclosed spaces', stats: { stealth: 20, territory: 'Horizontal' }},
        { value: 'C', text: 'Right in the middle of family activity - kitchen, living room', stats: { charisma: 15, territory: 'Social' }},
        { value: 'D', text: 'A sunny window spot where they can survey everything', stats: { wisdom: 10, territory: 'Surveillance' }}
      ]
    },
    {
      id: 'activity_time',
      ...
      options: [
        { value: 'A', text: 'Early morning - ready to start the day!', stats: { energy: 'Morning' }},
        { value: 'B', text: 'Afternoon - peak performance time', stats: { energy: 'Afternoon' }},
        { value: 'C', text: 'Evening - suddenly becomes super affectionate or energetic', stats: { energy: 'Evening' }},
        { value: 'D', text: 'They\'re pretty consistent throughout the day', stats: { energy: 'Balanced' }}
      ]
    }
  ];
  ...
  const openEndedQuestions = [
    {
      id: 'physical_feat',
      question: 'What\'s the most impressive physical thing your pet has ever done?'
    },
    {
      id: 'attention_move',
      question: 'Describe your pet\'s signature move when they want attention or treats.'
    },
    {
      id: 'weird_habit',
    ...
    {
      id: 'intelligence',
      question: 'Tell us about a time your pet surprised you with their intelligence or problem-solving.'
    },
    {
      id: 'superpower',
      question: 'If your pet were a superhero, what would their special power be?'
    }
  ];
        ...
        if (selectedOption?.stats) {
          Object.entries(selectedOption.stats).forEach(([stat, value]) => {
            if (typeof value === 'number' && baseStats.hasOwnProperty(stat)) {
              baseStats[stat] = Math.max(0, Math.min(100, baseStats[stat] + value));
            }
          });
        }
      }
...
⚔️  COMBAT MOVES:
${characterData.combatMoves.map(ability => 
  `• ${ability.name}\n  ${ability.stats}\n  ${ability.description}\n`
).join('')}

🌍 ENVIRONMENTAL POWERS:
${characterData.environmentalPowers.map(ability => 
  `• ${ability.name}\n  ${ability.stats}\n  ${ability.description}\n`
).join('')}

💬 SOCIAL SKILLS:
${characterData.socialSkills.map(ability => 
  `• ${ability.name}\n  ${ability.stats}\n  ${ability.description}\n`
).join('')}

🔮 PASSIVE TRAITS:
${characterData.passiveTraits.map(ability => 
  `• ${ability.name}\n  ${ability.stats}\n  ${ability.description}\n`
).join('')}
...
${characterData.timeModifiers.map(modifier => 
...
`;
    ...
    alert(`✨ ${name}'s character sheet downloaded! Perfect for printing or sharing.`);
  };
  ...
  const generateCharacterSheet = async () => {
    ...
    const behavioralInputs = openEndedQuestions.map(q => {
...
Create videogame-style abilities based on the pet's behaviors. Make ability names creative and memorable. Include game mechanics in the stats line. Return ONLY valid JSON with no other text.`;
  ...
  const drawRadarChart = (canvas, stats, theme) => {
    ...
    const statLabels = ['Wisdom', 'Cunning', 'Agility', 'Stealth', 'Charisma', 'Resolve'];
    const statValues = [stats.wisdom, stats.cunning, stats.agility, stats.stealth, stats.charisma, stats.resolve];
  ...
  const handleSubmit = () => {
    ...
    const requiredOpenEnded = openEndedQuestions.every(q => answers[q.id]?.trim());
      ...
      <div className={`min-h-screen bg-gradient-to-br ${theme.gradient} p-4`}>
        ...
        <div className="max-w-4xl mx-auto bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-xl">
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-white mb-2">{name.toUpperCase()}</h1>
          ...
          <div className="mb-8">
            <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-lg p-6">
              <h3 className="text-white font-bold text-xl mb-4 text-center">Core Attributes</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-lg p-6">
              <h3 className="text-white font-bold text-lg mb-4">Environmental Powers</h3>
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-lg p-6">
              ...
              {characterData.socialSkills.map((ability, index) => (
                <div key={index} className="bg-white/10 rounded-lg p-3 mb-3">
                  <div className="text-yellow-300 font-bold">{ability.name}</div>
                  <div className="text-white/60 text-sm italic">{ability.stats}</div>
                  <div className="text-white/80 text-sm">{ability.description}</div>
                </div>
              ))}
            ...
            <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-lg p-6">
              ...
              {characterData.passiveTraits.map((ability, index) => (
                <div key={index} className="bg-white/10 rounded-lg p-3 mb-3">
                  <div className="text-purple-300 font-bold">{ability.name}</div>
                  <div className="text-white/60 text-sm italic">{ability.stats}</div>
                  <div className="text-white/80 text-sm">{ability.description}</div>
                </div>
              ))}
          ...
          <div className="bg-red-500/20 backdrop-blur-md border border-red-500/30 rounded-lg p-6 mb-6">
            <h3 className="text-red-300 font-bold text-lg mb-2">⚠️ Critical Vulnerability</h3>
            <div className="text-white">
              <span className="font-bold">{characterData.weakness.name}:</span> {characterData.weakness.description}
            </div>
          </div>

          {characterData.timeModifiers && characterData.timeModifiers.length > 0 && (
            <div className="bg-white/10 backdrop-blur-md border border-white/30 rounded-lg p-6">
              <h3 className="text-white font-bold text-lg mb-4">Situational Modifiers</h3>
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white/10 backdrop-blur-md border border-white/20 rounded-2xl p-8 shadow-xl relative">
          ...
          <div className="space-y-6">
            <div className="mb-8">
              <div>
                <label className="block text-white font-semibold mb-2">Pet Name</label>
                <input
                  type="text"
                  value={petName}
            ...
            <div className="space-y-3">
              ...
              <textarea
                ...
                onChange={(e) => handleAnswerChange('stress_weakness', e.target.value)}
